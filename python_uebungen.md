...
1. [Methoden definieren und aufrufen](#1-methoden-definieren-und-aufrufen)
2. [Listen als Arrays](#2-listen-als-arrays)
3. [Tupel](#3-tupel)
4. [For-Schleifen mit iterierbaren Objekten](#4-for-schleifen-mit-iterierbaren-objekten)
5. [Mehrfachverzweigungen und geschachtelte Schleifen](#5-mehrfachverzweigungen-und-geschachtelte-schleifen)
6. [Zeichen und ASCII: `chr()` und `ord()`](#6-zeichen-und-ascii-chr-und-ord)
7. [Texte aufteilen mit `split()`](#7-texte-aufteilen-mit-split)
8. [Mehrfachzuweisung mit `map()`](#8-mehrfachzuweisung-mit-map)
9. [Generator-Ausdrücke und `all()`](#9-generator-ausdrücke-und-all)

---

## 1. Methoden definieren und aufrufen
...
4. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `setze_schiff(grid, schiff_felder)`, die alle Felder aus `schiff_felder` im Grid auf `"S"` setzt.
...
3. Gegeben: `treffer_liste = [(1,2), (1,3), (1,4)]`. Schreibe eine [for-Schleife](#4-for-schleifen-mit-iterierbaren-objekten), die jede Koordinate entpackt und `"Getroffen: Zeile X, Spalte Y"` ausgibt.
4. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `mittelpunkt(p1, p2)`, die zwei Koordinaten-[Tupel](#3-tupel) bekommt und den Mittelpunkt als neues Tupel zurückgibt. Beispiel: `mittelpunkt((2,4), (6,8))` → `(4, 6)`.

---

## 4. For-Schleifen mit iterierbaren Objekten
...
2. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `alle_felder(my_ships)`, die eine flache [Liste](#2-listen-als-arrays) aller belegten Felder zurückgibt. Beispiel: `[[(0,0),(0,1)], [(3,5)]]` → `[(0,0),(0,1),(3,5)]`.
...
## 5. Mehrfachverzweigungen und geschachtelte Schleifen
...
1. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `noten_text(note)`, die bei 1→`"sehr gut"`, 2→`"gut"`, 3→`"befriedigend"`, 4→`"ausreichend"`, sonst→`"nicht bestanden"` zurückgibt. Nutze `if/elif/else`.
2. Erstelle ein 5×5 [Spielfeld (als Liste von Listen)](#2-listen-als-arrays) (alle Felder `"."`). Schreibe zwei geschachtelte Schleifen, die jedes Feld ausgeben – zeilenweise.
...
4. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `finde_schiff(my_ships, zeile, spalte)`, die durch alle Schiffe iteriert und das Schiff zurückgibt, das das Feld `(zeile, spalte)` enthält. Nutze `break`. Gibt `None` zurück, wenn kein Schiff gefunden wird.
...
3. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `buchstabe_zu_spalte(buchstabe)`, die den umgekehrten Weg geht: `"A"` → 0, `"F"` → 5. Nutze `ord()`.
4. Erstelle eine [Liste](#2-listen-als-arrays) `spaltennamen`, die alle Buchstaben von A bis J enthält. Verwende eine List Comprehension mit `chr()`.

---

## 7. Texte aufteilen mit `split()`

`split(trennzeichen)` zerlegt einen String anhand eines Trennzeichens und gibt eine **[Liste](#2-listen-als-arrays)** von Teilstrings zurück.
...
3. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `parse_msg(msg)`, die eine Nachricht im Format `"BEFEHL:zeile,spalte"` entgegennimmt und ein [Tupel](#3-tupel) `(befehl, zeile, spalte)` zurückgibt (`zeile` und `spalte` als `int`).

---

## 8. Mehrfachzuweisung mit `map()`
...
3. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `parse_coords(text)`, die einen String wie `"3,5"` entgegennimmt und ein [Tupel](#3-tupel) `(int, int)` zurückgibt.
...
3. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `check_sunk(ship_cells, grid)`, die `True` zurückgibt, wenn alle Felder des Schiffes im Grid den Wert `"X"` haben. Nutze `all()` mit einem Generator-Ausdruck.
4. Schreibe eine [Funktion](#1-methoden-definieren-und-aufrufen) `check_all_sunk(my_ships, grid)`, die `True` zurückgibt, wenn alle Schiffe in `my_ships` versenkt sind. Nutze `all()` und rufe darin `check_sunk()` auf.
